...
parser.add_argument('-p', "--pred", metavar="FILENAME.conllu", required=True, dest="pred_filename",
...
def tp_count_feats(tok_pred, tok_gold, prf):
  ...
  """
    ...
    for key in pred_feats.keys():
        ...
        p_inc = int(pred_feats.get(key, None) is not None)
        prf[key]['p'] = prf[key]['p'] + p_inc
        prf['micro-avg']['p'] = prf['micro-avg']['p'] + p_inc
    for key in gold_feats.keys():
        t_inc = int(gold_feats.get(key, None) is not None)
...
def tp_count_deps(s_gold, s_pred, prf):
    """
    Count true positives, trues and positives for deps eval.
    Updates `prf` dict with counts from sentence `s_pred` and sentence `s_gold`
     Result is modification of `prf` dict, function does not return anything
    """
    deps_pred = s_pred["deps"]
    deps_gold = s_gold["deps"]

    prf['unlabeled']['t'] += len(deps_gold) if deps_gold else 0
    prf['unlabeled']['p'] += len(deps_pred) if deps_pred else 0
    prf['labeled']['t'] += len(deps_gold) if deps_gold else 0
    prf['labeled']['p'] += len(deps_pred) if deps_pred else 0

    if deps_pred:
        for pred_d, pred_h in deps_pred:
            for gold_d, gold_h in deps_gold:
                if pred_h == gold_h:
                    prf['unlabeled']['tp'] += 1
                    if pred_d == gold_d:
                        prf['labeled']['tp'] += 1
    ...
    if args.train_filename:
        ...
        if parsing:
            ...
            print("LAS on OOV {}: {:0.2f} ({:5}/{:5})".format(args.name_tag,
                                                              accuracy_oov_las, acc['correct_oov_las'],
                                                              acc['total_oov']))
    if prf:
...
if __name__ == "__main__":
    ...
    parsing = False
    for (s_gold, s_pred) in (
            zip(gold_corpus.readConllu(
        ...
        if args.name_tag in ["head", "deprel"]:  # Considered as LAS/UAS or UF1/LF1 eval
            args.name_tag = "head"
            parsing = True
            ...
            if not args.upos_filter or tok_gold['upos'] in args.upos_filter:
                if train_vocab:
                    ...
                    if tok_gold[args.name_feat] not in train_vocab_feat:
                        ...
                        oov = False
                if tok_gold[args.name_tag] == tok_pred[args.name_tag] and args.name_tag not in ['deps']:
                    acc['correct_tokens'] += 1
                    if train_vocab and oov:
                        acc['correct_oov'] += 1

                # LAS ignores subrelations, as usual in CoNLL17/18 eval scripts => TO BE REVIEWED
                ...
                if args.name_tag == 'deps':
                    tp_count_deps(tok_gold, tok_pred, prf)
